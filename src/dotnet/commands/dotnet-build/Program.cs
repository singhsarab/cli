...
namespace Microsoft.DotNet.Tools.Build
{
    public class BuildCommand
    {
        ...
        private static bool OnExecute(IEnumerable<string> files, IEnumerable<NuGetFramework> frameworks, CompilerCommandApp args)
        {
            ...
            var contexts = ResolveRootContexts(files, frameworks);
        ...
        private static IEnumerable<ProjectContext> ResolveRootContexts(IEnumerable<string> files,
            IEnumerable<NuGetFramework> frameworks)
        {
            foreach (var file in files)
            {
                ...
                {
                    throw new InvalidOperationException($"Project '{file}' does not support framework:" +
                                                        string.Join(", ",
                                                            frameworks.Select(fx => fx.DotNetFrameworkName)));
                }

                foreach (var framework in selectedFrameworks)
                {
                    yield return ProjectContext.Create(file, framework);
                }
            }
        }
    }
